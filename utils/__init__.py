"""Package to provide utility classes and functions for the insect-detect repository.

Source:   https://github.com/maxsitt/insect-detect
License:  GNU GPLv3 (https://choosealicense.com/licenses/gpl-3.0/)
Author:   Maximilian Sittinger (https://github.com/maxsitt)
Docs:     https://maxsitt.github.io/insect-detect-docs/

Modules:
    general: Utility functions for data management and general functionality.
    log: Utility functions to print information or save to log file.
    oak_cam: Utility functions for Luxonis OAK camera control.
    post: Utility functions for image post-processing.
    wittypi: Utility class for reading status information from Witty Pi 4 L3V7.
"""

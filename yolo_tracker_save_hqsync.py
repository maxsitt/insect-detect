...
- shut down Raspberry Pi without recording if current PiJuice battery charge level or
  free disk space are lower than the specified thresholds (default: 10% and 100 MB)
...
def save_logs():
    """Write recording ID, time, RPi CPU + OAK chip temperature, RPi available
    memory (MB) + CPU utilization (%) and PiJuice battery info + temp to .csv.
    """
    try:
...
with dai.Device(pipeline, maxUsbSpeed=dai.UsbSpeed.HIGH) as device:
    ...
    logger.info("Rec ID: %s | Rec time: %s min | Charge level: %s%%", rec_id, int(REC_TIME / 60), chargelevel_start)
    ...
    try:
        ...
        while time.monotonic() < start_time + REC_TIME and disk_free > MIN_DISKSPACE and len(chargelevel_list) < 10:
            ...
            # Update charge level (return "99" if not readable and write to list if lower than threshold)
